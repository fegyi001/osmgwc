...
drop table if exists osm.province;
create table osm.province(
  id serial not null primary key,
  osm_id integer,
  name text,
  uppername text,
...
create index gix_province on osm.province using gist(geom);
delete from osm.province;
insert into osm.province(osm_id, name, uppername, geom) 
    SELECT planet_osm_polygon.osm_id,
      planet_osm_polygon.name as name,
      upper(planet_osm_polygon.name) AS uppername,
      st_multi(planet_osm_polygon.way)::geometry(MultiPolygon, 3857) as way
...
insert into osm.county(osm_id, name, uppername, geom) 
	SELECT planet_osm_polygon.osm_id, 
    	...
    	st_multi(planet_osm_polygon.way)::geometry(MultiPolygon, 3857) as way
   	FROM planet_osm_polygon
  	WHERE (planet_osm_polygon.place = 'county'::text OR planet_osm_polygon.admin_level = '5'::text) AND planet_osm_polygon.boundary = 'administrative'::text;
delete from osm.county where not st_intersects(st_centroid(geom), (select geom from osm.country limit 1));

drop table if exists osm.urban_district;
create table osm.urban_district(
  id serial not null primary key,
  osm_id integer,
  name text,
  uppername text,
...
create index gix_urban_district on osm.urban_district using gist(geom);
delete from osm.urban_district;
insert into osm.urban_district(osm_id, name, uppername, geom) 
  SELECT planet_osm_polygon.osm_id, 
    planet_osm_polygon.name, 
    upper(planet_osm_polygon.name) AS uppername,
    st_multi(planet_osm_polygon.way)::geometry(MultiPolygon, 3857) as way
...
delete from osm.urban_district where not st_intersects(st_centroid(geom), (select geom from osm.country limit 1));
...
insert into osm.district(osm_id, name, uppername, way_area, geom) 
  SELECT planet_osm_polygon.osm_id, 
    planet_osm_polygon.name, 
    upper(planet_osm_polygon.name) AS uppername,
    planet_osm_polygon.way_area,
...
insert into osm.city(osm_id, name, uppername, way_area, geom) 
  SELECT planet_osm_polygon.osm_id, 
    planet_osm_polygon.name, 
    upper(planet_osm_polygon.name) AS uppername,
    planet_osm_polygon.way_area,
...
drop table if exists osm.vilage;
create table osm.vilage(
  id serial not null primary key,
  osm_id integer,
  name text,
  uppername text,
...
create index gix_vilage on osm.vilage using gist(geom);
delete from osm.vilage;
insert into osm.vilage(osm_id, name, uppername, way_area, geom) 
  SELECT planet_osm_polygon.osm_id, 
    planet_osm_polygon.name, 
    upper(planet_osm_polygon.name) AS uppername,
    planet_osm_polygon.way_area,
...
delete from osm.vilage where not st_intersects(st_centroid(geom), (select geom from osm.country limit 1));

drop table if exists osm.urban_area;
create table osm.urban_area(
  id serial not null primary key,
  osm_id integer,
  name text,
  uppername text,
...
create index gix_urban_area on osm.urban_area using gist(geom);
delete from osm.urban_area;
insert into osm.urban_area(osm_id, name, uppername, geom) 
  SELECT planet_osm_polygon.osm_id, 
    planet_osm_polygon.name,  
    upper(planet_osm_polygon.name) AS uppername,
    st_multi(planet_osm_polygon.way)::geometry(MultiPolygon, 3857) as way
...
delete from osm.urban_area where not st_intersects(st_centroid(geom), (select geom from osm.country limit 1));

drop table if exists osm.neighborhood;
create table osm.neighborhood(
  id serial not null primary key,
  osm_id integer,
  name text,
  uppername text,
...
create index gix_neighborhood on osm.neighborhood using gist(geom);
delete from osm.neighborhood;
insert into osm.neighborhood(osm_id, name, uppername, geom) 
  SELECT planet_osm_polygon.osm_id, 
    planet_osm_polygon.name,  
    upper(planet_osm_polygon.name) AS uppername,
    st_multi(planet_osm_polygon.way)::geometry(MultiPolygon, 3857) as way
...
delete from osm.neighborhood where not st_intersects(st_centroid(geom), (select geom from osm.country limit 1));
